...
| `--color` | string | auto | 色付け制御（never/always/auto） |
...
#### 色付け決定ロジック
1. **NO_COLOR環境変数チェック**: `NO_COLOR`環境変数が存在し、かつ空文字列でない場合は色付けを無効化
2. **--colorオプション適用**: 
   - `never`: 色付けを無効化
   - `always`: 色付けを強制有効化
   - `auto`: 出力先が端末の場合のみ色付けを有効化
3. **NO_COLOR最優先**: `NO_COLOR`環境変数が設定されている場合、`--color`オプションの値に関わらず色付けを無効化
...
#### 端末検出
- 標準エラー出力（`stderr`）が端末（TTY）に接続されているかを判定
- パイプやリダイレクトされている場合は非端末として扱う
- 実装: [mattn/go-isatty](https://github.com/mattn/go-isatty)ライブラリを使用

#### 実装技術
- 色付け機能: [fatih/color](https://github.com/fatih/color)による色付きログ出力
- 端末検出: [mattn/go-isatty](https://github.com/mattn/go-isatty)による端末判定
